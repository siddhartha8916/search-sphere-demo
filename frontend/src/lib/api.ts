...
export async function getAttachments(): Promise<AttachmentsResponse> {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({ detail: 'Failed to fetch attachments' }))
    console.error('Attachments API error:', errorData)
    throw new ApiError(response.status, errorData.detail || 'Failed to fetch attachments')
  }
